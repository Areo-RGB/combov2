...
export class WebRTCService {
  ...
  sendMessage(peerId: string, message: Message): void {
    ...
    if (channel && channel.readyState === 'open') {
      ...
      console.warn(
        `Data channel for ${peerId} is not open. State: ${channel?.readyState}. Queuing message.`
      );
  ...
  private createPeerConnection(peerId: string): RTCPeerConnection {
    ...
    pc.onconnectionstatechange = () => {
      if (this.peerConnections.has(peerId)) {
        // Avoid callback on closed connections
      ...
      if (activePair && activePair.localCandidateId && activePair.remoteCandidateId) {
        ...
        if (localCandidate && remoteCandidate) {
          ...
          if (localCandidate.candidateType === 'host' && remoteCandidate.candidateType === 'host') {
            ...
            localCandidate.candidateType.includes('flx') ||
            remoteCandidate.candidateType.includes('flx')
          ) {
