...
export class DisplayHostComponent implements OnInit, OnDestroy {
  ...
  webrtcStatus = computed(() => {
    ...
    const connecting = clients.some(
      (c) => c.connectionState === 'connecting' || c.connectionState === 'new'
    );
    if (connecting) {
      return {
        text: 'Connecting...',
  ...
  ngOnInit(): void {
    ...
    this.clientListenerCleanup = this.firebaseService.listenForClients(
      this.sessionId(),
      (deviceId) => {
        ...
        if (!this.clients().some((c) => c.deviceId === deviceId)) {
          ...
          this.clients.update((clients) => [
            ...clients,
            { deviceId, livepool: 0, lastReactionTime: 0, connectionState: 'new' },
          ]);
        }
      }
    );
      ...
      if (state === 'failed' || state === 'closed') {
        ...
        setTimeout(() => {
          this.clients.update((currentClients) =>
            currentClients.filter((c) => c.deviceId !== peerId)
          );
  ...
  private handleClientMessage(deviceId: string, message: Message): void {
    if (message.type === 'clientStateUpdate') {
      ...
      this.clients.update((clients) =>
        clients
          .map((c) => (c.deviceId === deviceId ? { ...c, ...newState, deviceId } : c))
  ...
  private playWarningSoundForDevice(deviceId: string): void {
    ...
    player
      .play()
      .catch((err) => console.error(`Warning audio playback failed for ${deviceId}:`, err));
  }
  ...
  private checkAndAnnounceThresholds(
    deviceId: string,
    oldValue: number,
    ...
    for (const threshold of thresholds) {
      if (
        ...
        newState.livepool <= threshold.value &&
